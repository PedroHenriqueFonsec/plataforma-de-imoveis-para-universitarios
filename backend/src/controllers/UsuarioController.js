...
module.exports = {
  async cadastrar(req, res) {
    const { nome, email, senha, telefone, tipo, foto } = req.body;
    ...
    try {
      ...
      if (foto) {
        const upload = await cloudinary.uploader.upload(foto, {
  ...
  async atualizarPerfil(req, res) {
    const { nome, email, telefone, senhaAtual, novaSenha, g } = req.body;
